...
Module["onRuntimeInitialized"] = function onRuntimeInitialized() {
    ...
    */
    function Database(data) {
        ...
        // (created by create_function or create_collation call)
        this.functions = {};
    }
    ...
    */
    ...
    /** Register a custom collation with SQLite
    @example Register a simple collation
        db.create_collation("length", function (x, y) {
        ...
        db.exec(
            `SELECT 'two' as c
            UNION SELECT 'three' as c
            UNION SELECT 'four' as c
            ORDER by c COLLATE length`
        `) // order is 'two', 'four', 'three'

    @param {string} name the name of the collation as referenced in
    SQL statements.
    @param {func} the comparison function to be executed---takes two arguments
    and returns -1 if the first is smaller, 1 if the first is greater, 0 if
    they're equal
    @return {Database} The database object. Useful for method chaining
     */
    Database.prototype["create_collation"] = function create_collation(
        name,
        func
    ) {
        function wrapped_func(appdata, len1, str1, len2, str2) {
          var result;
          var // in progress
        }
        ...
        // int wrapped(void *appdata, int len1, const void* str1, int len2, const void* str2)
        var func_ptr = addFunction(wrapped_func, "viii");
        this.functions[name] = func_ptr;
